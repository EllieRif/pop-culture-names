...
babynames %>%
  ...
  labs(title = "Names Introduced by Harry Potter", subtitle = "First appearence or major character revelation (Snape)")
...
babynames %>%
  filter(name %in% c( "Jill", "Jillian"), sex == "F") %>%
  filter(year>=1950) %>%
  ggplot() +
  geom_point(mapping = aes( x=year, y = n)) + 
  facet_wrap(~name, scales = "free_y") + 
